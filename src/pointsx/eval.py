...
def main() -> None:
    ...
    if args.grid:
        ...
        if args.fit_offsets:
            ...
            # Two views: all display IDs, then filtered to the 4 circumferences
            # the bias scales actually correct (so you see the fit's pure effect).
            sex_by_subj: dict[str, str] = {}
            for r in error_rows:
                sex_by_subj[r.subject_id] = r.sex

            def _print_subject_delta(title: str, mid_filter: set[str] | None) -> None:
                print(f"\n=== {title} ===")
                ...
                for r in error_rows:
                    if mid_filter is None or r.measurement_id in mid_filter:
            ...
            _print_subject_delta("Per-subject before/after fit (all)", None)
            _print_subject_delta(
                "Per-subject before/after fit (Обхвати only)",
                set(FIT_TARGET_IDS),
            )
