...
- Крок 10 (`refactor/webui-10-claude-rules`): `.claude/CLAUDE.md` — структура `src/webui` і `static/js`, правила шарів
  (бекенд: тонкі роутери, сервіси без FastAPI з `AppError`, 503 «моделі не завантажені» лише в сервісі, важкі імпорти
  всередині функцій, env лише через `config.py`; фронтенд: мережа лише через `api/`, рядки лише в `i18n/`, стабільні
  фасади `session.js` / `tailoring.js`, `?v=` при зміні графа модулів) і чекліст рефакторингу з воротами кроків 1–9.
  Виправлено застарілі посилання (`envelope/corrections.py`, моделі не YOLO11n). План виконано повністю; поза ним лишився
  лише `dataset/app.js` (після узгодження з MaksShu).
